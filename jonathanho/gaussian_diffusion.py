...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        if loss_type == 'mse':
            ...
            raise ValueError(f'Nonsupported type of loss for diffusion: {loss_type}.')

        # Use float64 for accuracy.
        betas = get_named_beta_schedule('linear', num_diffusion_steps)
    ...
    def q_mean_variance(self, x_start, t):
        ...
        :param x_start: the [B, T, D] tensor of noiseless inputs.
    ...
    def p_mean_variance(
        ...
        :param model: the model, which takes a signal and a batch of timesteps
                      as input.
        :param x: the [B, T, D] tensor at time t.
        :param t: a 1-D Tensor of timesteps. Here, 0 means one step.
    ...
    def p_sample_loop(
        ...
        :param shape: the shape of the samples, (B, T, D).
    ...
    def _compute_kl_loss_bpd(
            self, model, x_start, x_t, t, x_start_mean, x_start_log_var,
            clip_denoised=False, model_kwargs=None, noise=None, denoised_fn=None,
    ):
        if model_kwargs is None:
            model_kwargs = {}
        # Note: t is more appropriately interpreted as a zero-based index.
        ...
        kl_0 = normal_kl(
            x_start_mean, x_start_log_var, out["mean"], out["log_variance"] # x_start_log_var
        )
        kl_0 = mean_flat(kl_0) / np.log(2.0)
        kl = th.where(mask_0, kl_0, kl)
        ...
        # Weiran: monitor the value of each loss terms.
        return {"pred_xstart": out["pred_xstart"], 'kl': kl, 'kl_T': kl_T, 'loss': kl + kl_T}

    def _compute_mse_loss_bpd(
            self, model, x_start, x_t, t, x_start_mean, x_start_log_var,
            clip_denoised=False, model_kwargs=None, noise=None, denoised_fn=None,
    ):
        if model_kwargs is None:
        ...
        mse_0 = mean_flat((x_start_mean - model_out_x_start) ** 2)
        mse = th.where(mask_0, mse_0, mse)

        # import pdb;pdb.set_trace()
        out_mean, _, _ = self.q_mean_variance(x_start, th.LongTensor([self.num_timesteps - 1]).to(x_start.device))
        mse_T =  mean_flat(out_mean ** 2)

        # Weiran: monitor the value of each loss terms.
        return {"pred_xstart": model_out_x_start, 'mse': mse, 'mse_T':mse_T, 'loss': mse + mse_T}
    ...
    def training_losses_e2e(self, model, inputs, t, model_kwargs=None, noise=None):
        """
        Compute training losses for a single timestep.

        :param model: the model to evaluate loss on.
        :param inputs: input features, shape [B, D]
        :param t: a batch of timestep indices.
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning.
        :param noise: if specified, the specific Gaussian noise to try to remove.
        :return: a dict with the key "loss" containing a tensor of shape [B].
                 Some mean or variance settings may also have other keys.
        """
        # import pdb;pdb.set_trace()
        x_start_mean = inputs
        # Weiran: use some artificial variance for x_start.
        std = _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod,
                                   th.tensor([0]).to(x_start_mean.device),
                                   x_start_mean.shape)
        x_start_log_var = 2 * th.log(std)
        # Weiran: originally x_start = self.get_x_start(x_start_mean, std)
        ...
        if self.loss_type == LossType.KL:
            terms = self._compute_kl_loss_bpd(
                model=model,
                x_start=x_start,
                x_t=x_t,
                t=t,
            ...
            # Mse terms.
            terms = self._compute_mse_loss_bpd(
                model=model,
                x_start=x_start,
                x_t=x_t,
                t=t,
            ...
            raise ValueError('Unsupported diffusion loss type.')

        # print('diffusion mse or kl: %f' % terms['loss'].mean())
        print('diffusion total loss: %f' % terms['loss'].mean())
        # print('loss mse: %f' % terms['mse'].mean())
        # print('loss mse_T: %f' % terms['mse_T'].mean())
        # print('loss kl: %f' % terms['kl'].mean())
        # print('loss kl_T: %f' % terms['kl_T'].mean())
        return terms
...
#
#         :param x_start: the [B, T, D] tensor of inputs.
#         :return: a batch of [B] KL values (in bits), one per batch element.
#         """
...
#     def calc_bpd_loop(self, model, inputs, x_start, clip_denoised=False, model_kwargs=None, denoised_fn=None):
